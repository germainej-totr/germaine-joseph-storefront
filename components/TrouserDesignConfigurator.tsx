...
/** Selections map: option.key -> chosen value (undefined = not yet selected) */
export type TrouserSelections = Record<string, string | undefined>;

export interface TrouserDesignConfiguratorProps {
  ...
  onChange: (next: TrouserSelections, priceDelta: number) => void;
  /** Override the option-set schema (defaults to trouser-core-v1) */
  ...
  onComplete?: (selections: TrouserSelections, totalPrice: number) => void;
...
function PriceDelta({ amount, symbol }: { amount: number; symbol: string }) {
  if (!amount)
    return <div className="mt-2 text-xs text-gray-400">Included</div>;
  return (
...
// ---------------------------------------------------------------------------
// DropdownSelect — uiHint: "dropdown"
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// TrouserDesignConfigurator — main export
// ---------------------------------------------------------------------------

export default function TrouserDesignConfigurator({
  value,
  onChange,
...
}: TrouserDesignConfiguratorProps) {
  const visibleOptions = useMemo(
    () => getVisibleTrouserOptions(value),
    [value],
  );
  ...
  const handleSelect = useCallback(
    (optionKey: string, nextValue: string) => {
      const next: TrouserSelections = { ...value, [optionKey]: nextValue };
      ...
      const newVisible = getVisibleTrouserOptions(next);
          ...
          {title && (
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Configure the design details for your trousers.
              </p>
            </div>
          )}
          {basePrice > 0 && (
