...
/**
 * A19: MTM Order Review Card
 *
 * Full-page review view combining the design, fit, booking, and
 * fabric snapshots with the reviewer checklist and payload inspector.
...
function MoneyAmount({ pence }: { pence?: number }) {
  ...
  return <span>£{(pence / 100).toFixed(2)}</span>;
}
    ...
    <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-200 mb-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          {summary.category.toUpperCase()} · payload {summary.version} ·{' '}
  ...
  return (
    <SectionCard title="Design Snapshot">
      ...
      <FieldRow
        label="Total price"
        value={
          design.pricingTotal != null
            ? `£${(design.pricingTotal / 100).toFixed(2)}`
            : undefined
        }
      />
      <FieldRow
        label="Configuration valid"
        value={design.isValid ? 'Yes' : `No — ${design.validationErrors?.join(', ') ?? ''}`}
      />
...
type Tab = 'overview' | 'checklist' | 'inspector';
...
export function MtmOrderReviewCard({ summary }: MtmOrderReviewCardProps) {
